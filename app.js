...
timeEl.textContent = now.toLocaleTimeString('en-US', {
  ...
  // ---- Unlock sound easter egg (kiyosumi - disorder, disbelief, via SoundCloud) ----
  let scWidget = null, scWidgetReady = false;
  function ensureUnlockWidget(onReady, attemptsLeft){
    attemptsLeft = attemptsLeft ?? 20;
    const iframe = document.getElementById('unlockPlayer');
    if (!iframe) { onReady?.(null); return; }
    if (!window.SC || !window.SC.Widget) {
      if (attemptsLeft <= 0) { onReady?.(null); return; }
      setTimeout(() => ensureUnlockWidget(onReady, attemptsLeft - 1), 150);
      return;
    }
  ...
  function unlock(e){
    ...
    setTimeout(() => { loader.style.display = 'none'; stopUnlockTrack(); }, 550);
